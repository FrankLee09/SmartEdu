<template>
  <div class="user-page">
    ...
    <el-dialog title="教师信息" v-model="data.formVisible" width="50%" destroy-on-close>
...
const toMain = (course: any) => {
  router.push({ path: `/manager/courseManager` });
};
...
const loadCourses = async () => {
  ...
  try {
    ...
    filteredCourseList.value = courseList.value.filter(course => course.teacherId === data.user.id);
    console.log('课程数据加载成功', courseList.value);
  } catch (error) {
